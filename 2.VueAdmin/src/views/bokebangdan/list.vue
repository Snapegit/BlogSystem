	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.bokefenlei" 
								placeholder="博客分类"
								>
								<el-option v-for="item in bokefenleiLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
					<div class="search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('bokebangdan','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('bokebangdan','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('bokebangdan','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('bokebangdan','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('bokebangdan','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('bokebangdan','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.bokefenlei&&searchQuery.value.bokefenlei!=''){
			params['bokefenlei'] = searchQuery.value.bokefenlei
		}
	...
	//搜索
	const bokefenleiLists = ref([])
	const getbokefenleiLists = () => {
		context?.$http({
			url: 'option/bokefenlei/bokefenlei',
			method:'get'
		}).then(res=>{
			bokefenleiLists.value = res.data.data
		})
	}
	getbokefenleiLists()
	const searchClick = () => {
	...
	const commentClick=(id)=>{
		context?.$router.push('/discussbokebangdan?refid=' + id)
	}
	//初始化
	const init = () => {
		getList()
