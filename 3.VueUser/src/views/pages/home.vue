	...
	<div>
		<div class="home_box">
			...
			<div class="recomList_view">
				<div class="recomList_title">博客榜单推荐</div>
				<div class="recommend_list_four">
					<div class="recommend_item" v-for="(item,index) in bokebangdanRecomList" :key="index" @click="detailClick('bokebangdan',item.id)">
						<div class="recommend_img_box">
							<img class="recommend_img" v-if="isHttp(item.tupian)" :src="item.tupian.split(',')[0]" alt="">
							<img class="recommend_img" v-else :src="item.tupian?$config.url + item.tupian.split(',')[0]:''" alt="">
						</div>
						<div class="recommend_content">
							<div class="recommend_title1">
								博客分类：{{item.bokefenlei}}
							</div>
							<div class="recommend_title2">
								用户姓名：{{item.yonghuxingming}}
							</div>
						</div>
					</div>
				</div>
				<div class="recommend_more_view" @click="moreClick('bokebangdan')">
...
<script setup>
	...
	//博客榜单推荐
	const bokebangdanRecomList = ref([])
	const getbokebangdanRecomList = () => {
		let autoSortUrl = 'bokebangdan/autoSort'
		if(context?.$toolUtil.storageGet('frontToken')){
			autoSortUrl = "bokebangdan/autoSort2"
		}
		context?.$http({
			url: autoSortUrl,
			method: 'get',
			...
			bokebangdanRecomList.value = res.data.data.list
		})
	}
	//公告信息弹窗
