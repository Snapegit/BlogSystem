	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0","margin":"0 auto","overflow":"hidden","borderRadius":"4px","background":"none","width":"1200px","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('bokebangdan','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
				...
				<div class="data_view">
					<div class="data_item animation_box" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						...
						<div class="data_content">
							<div class="data_title">博客分类：{{item.bokefenlei}}</div>
							<div class="data_title">用户姓名：{{item.yonghuxingming}}</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/bokebangdanAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.bokefenlei = categoryList.value[categoryIndex.value]
		}
	...
	const detailClick = (id) => {
		router.push('bokebangdanDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
